...
The `outputs/tables/eda_decisions.csv` audit trail captures every analytical decision with one of three statuses: **Validated (Phase 2)** for EDA-confirmed decisions, **Planned (DP)** for design commitments awaiting empirical validation, and **Open** for questions still under investigation. Each row traces back to a specific notebook section, follow-up query, or Dissertation Proposal Chapter 3 reference.

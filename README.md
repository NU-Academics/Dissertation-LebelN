...
The proposal defended successfully in May 2026. All five research questions are modeled and frozen, the cross-cutting evaluation is complete, and the analysis is declared frozen at tag `analysis_frozen/v1`; Chapter 4 and 5 prose work proceeds against the frozen artifacts, and repository changes now require an errata tag and a successor freeze tag. Each phase below maps to specific notebooks in `notebooks/` and to modules extracted into `src/`.
...
- **Phase 6 (Deployment):** Complete. **Chapter 4 regeneration pipeline** (`notebooks/19_tables_figures.py`): the single source of truth for the committee-facing Chapter 4 artifact set. It reads only frozen artifacts (resolving tables from the tracked snapshots `google_block/`, `backblaze_block/`, and `evaluation/` before any working copy, and reporting any working-copy drift), re-renders every figure whose data live in frozen tables with one shared style, carries forward the figures whose per-observation data were never tabled byte for byte with source checksums, and pins the SHAP figure stamp from the evaluation snapshot's run metadata. It writes 14 tables to `outputs/tables/chapter4_final/`, 45 figures to `outputs/figures/chapter4_final/`, and `outputs/manifest_chapter4.json` recording the generating commit, library versions, and SHA-256 checksums of every input and output. Regeneration is hermetic: deleting the two `chapter4_final/` directories and re-running the notebook reproduces every artifact byte for byte, verified across independent runs. The exact dependency environment is recorded in `requirements_lock.txt`, and the reporting follows the TRIPOD+AI checklist completed with the manuscript materials. Decisions log row V59 records the freeze; the repository is tagged `analysis_frozen/v1`.
...
├── requirements_lock.txt      Exact dependency versions of the frozen analysis environment (pip freeze)
...
│                                     19 Chapter 4 tables/figures regeneration)
│
...
- **Execution:** Google Colab with the high-RAM T4 GPU runtime (16 GB VRAM, ~51 GB system RAM). Notebooks are stored as Jupytext `.py` files; `.ipynb` is generated on demand in Colab and is gitignored.

...
### Remaining Phase 5 onward (planned)

The remaining notebooks cover cross-cutting hypothesis testing (notebook 17), the Backblaze portion of the feature ablation and the sensitivity branches (notebook 18), and the Chapter 4 tables/figures regeneration pipeline (notebook 19). Each will get its own narrative-first treatment, with logic moving into `src/` only after the notebook validates it.
